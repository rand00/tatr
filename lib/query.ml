...
module Tree = struct
  ...
  (*> goto goo; reuse logic, but case on new default-param*)
  let match_subtree (query:t) tree : _ Tree.t =
    let rec aux ancestor_matched acc_path_matches = function
      ...
      | Tree (v, children) ->
        ...
        if found_match then (
          (*> goto goo try to reuse same function - almost no code changed from match_matchtree*)
          (*> Note: the differing section from 'match_matchtree'*)
          begin
